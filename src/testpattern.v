...
//-------------------------------------------------------------
wire [11:0] hbp = I_h_bporch;
wire [11:0] hsw = I_h_sync;
wire [11:0] hfp = I_h_total - hsw - hbp - I_h_res;
wire [11:0] vbp = I_v_bporch;
wire [11:0] vsw = I_v_sync;
wire [11:0] vfp = I_v_total - vsw - vbp - I_v_res;

assign  Pout_de_w = (H_cnt >= (hfp+hsw+hbp)) & (V_cnt >= (vfp+vsw+vbp));
assign  Pout_hs_w = !((H_cnt >= hfp) & (H_cnt < (hfp+hsw)));
assign  Pout_vs_w = !((V_cnt >= vfp) & (V_cnt < (vfp+vsw)));
...
always @(posedge I_pxl_clk or negedge I_rst_n) begin
    ...
    else if (1)     Gray <= {De_hcnt[7:0],De_hcnt[7:0],De_hcnt[7:0]};
end

always @(posedge I_pxl_clk or negedge I_rst_n) begin
    if (!I_rst_n)   Gray_d1 <= 24'd0;
    else if (1)     Gray_d1 <= Gray;
end
...
//============================================================
assign Data_sel = (I_mode[2:0] == 3'b000) ? Single_color
                : (I_mode[2:0] == 3'b001) ? Net_grid
                : (I_mode[2:0] == 3'b010) ? Gray_d1
                : (I_mode[2:0] == 3'b011) ? Color_bar
                : (I_mode[2:0] == 3'b100) ? RED
                : (I_mode[2:0] == 3'b101) ? GREEN
                : (I_mode[2:0] == 3'b110) ? BLUE
                : (I_mode[2:0] == 3'b111) ? 24'h55_55_55
                : BLUE;

//---------------------------------------------------
always @(posedge I_pxl_clk or negedge I_rst_n) begin
    if (!I_rst_n)           Data_tmp <= 24'd0;
//    else if (De_hcnt<10)    Data_tmp <= 24'h55AA55;  // For DEBUG
//    else if (De_hcnt>1913)  Data_tmp <= 24'hAA55AA;  // For DEBUG
    else if (1)             Data_tmp <= Data_sel;
end
